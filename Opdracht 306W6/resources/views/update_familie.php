...
if (isset($_GET['id'])) {
    ...
    $familielidResultaat = executePreparedStatement($conn, $familielidSql, $familieId);

    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        // Haal de formuliergegevens op voor familie
        $familieNaam = $_POST['familieNaam'];
        $familieAdres = $_POST['familieAdres'];
        ...
        if (!empty($naamFamilielid)) {
            ...
            if (!$familielidUpdateResultaat) {
                echo "Er is een fout opgetreden bij het bijwerken van het familielid.";
                exit();
            }
        }
        ...
        // Controleer of er een familielid moet worden verwijderd
        if (isset($_POST['verwijder_familielid'])) {
            $familielidId = $_POST['familielidId'];
            $status = 'verwijderd';
            $verwijderFamilielidSql = "UPDATE familielid SET status = ? WHERE id = ?";
            $verwijderFamilielidResultaat = executePreparedStatement($conn, $verwijderFamilielidSql, $status, $familielidId);

            if (!$verwijderFamilielidResultaat) {
